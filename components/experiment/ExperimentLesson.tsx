...
function isInteractive(step: Step): boolean {
  return step.kind !== 'concept' && step.kind !== 'concept-visual';
}
...
export default function ExperimentLesson() {
  ...
  const handleCta = () => {
    ...
    if (needsCheck) {
      ...
      if (
        isScoreable(step) &&
        !isAttemptCorrect(step, attempt) &&
        !scoredSteps.has(index)
      ) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white dark:bg-gray-950">
      ...
      <header className="relative px-4 py-4">
        ...
        <div
          className={`absolute right-4 top-1/2 -translate-y-1/2 inline-flex items-center gap-2 h-[42px] px-4 rounded-xl bg-white dark:bg-gray-800 text-[#4b4b4b] dark:text-gray-300 border-gray-300 dark:border-gray-900 border-b-gray-300 dark:border-b-gray-900 ${depthBase} origin-center transition-transform duration-200 ${livesPulse ? 'scale-125' : 'scale-100'}`}
