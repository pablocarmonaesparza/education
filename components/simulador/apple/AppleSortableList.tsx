...
/**
 * AppleSortableList · primitive de lista re-ordenable por arrastre.
 *
 * Promovido al design system desde `app/exercise-lab/_shared/SortableList.tsx`.
 * Markup EXACTO del original de Codex — fuente única acá; el lab lo consume vía
 * el shim `_shared/SortableList.tsx`. Hereda los tokens del ancestro
 * `.simulador-root`.
 *
...
export interface AppleSortableListProps<T> {
  items: T[];
...
export function AppleSortableList<T>({
  items,
  getItemKey,
  onReorder,
  renderItem,
  className = "space-y-2",
}: AppleSortableListProps<T>) {
