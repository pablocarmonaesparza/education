...
5. **Actualiza `docs/memory/INDEX.md`** con `Edit`: añade una línea bajo la sección correcta, formato:
   ```
   - [título](<tipo>_<slug>.md) — YYYY-MM-DD — hook de una línea
   ```
   Mantén orden cronológico descendente dentro de cada sección (lo nuevo arriba).

6. **Confirma al usuario** qué guardaste. Formato:
